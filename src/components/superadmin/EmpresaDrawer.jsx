...
const BLANK = {
  ...
  plan_tier: "Profissional", subscription_status: "Trial",
  admin_nome: "", admin_email: "",
};

export default function EmpresaDrawer({ open, onClose, onSaved, tenant }) {
  const isEdit = !!tenant;
    ...
    if (open) {
      if (isEdit) {
        setForm({
          ...
          plan_tier: tenant.plan_tier || "Profissional",
  ...
  const handleSave = async () => {
    ...
    if (!isEdit && !form.admin_email.trim()) { toast.error("E-mail do admin é obrigatório."); return; }
    ...
    if (isEdit) {
      ...
      toast.success(`Empresa "${form.nome_fantasia}" atualizada!`);
    } else {
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent className="w-full sm:max-w-md bg-card/95 backdrop-blur-xl border-border/50 overflow-y-auto">
        <SheetHeader>
          <SheetTitle className="font-heading">{isEdit ? "Editar Empresa" : "Nova Empresa (Produtora)"}</SheetTitle>
        </SheetHeader>
        ...
        <div className="space-y-6 mt-6 pb-6">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button className="flex-1" onClick={handleSave} disabled={saving}>
              {saving ? (isEdit ? "Salvando..." : "Criando...") : (isEdit ? "Salvar Alterações" : "Criar Empresa")}
            </Button>
          </div>
        </div>
      </SheetContent>
