...
export default function ProposalPrintView({ proposal, client, tenant, onClose }) {
  ...
  const docProps = { proposal, client, tenant, items, issueDate, validityDate, subtotal, discountAmt };
...
function PrintDocument({ proposal, client, tenant, items, issueDate, validityDate, subtotal, discountAmt }) {
  ...
  return (
    <div style={{ fontFamily: "Montserrat, sans-serif", color: "#111827" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", paddingBottom: "24px", borderBottom: "2px solid #e5e7eb", marginBottom: "32px" }}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          {proposal?.payment_method && (
            <p style={{ fontSize: "11px", color: "#6b7280", margin: "2px 0 0" }}>Pagamento: {proposal.payment_method}{proposal.installments && proposal.payment_method === "Parcelado" ? ` (${proposal.installments}x)` : ""}</p>
          )}
      ...
      <div style={{ marginBottom: "32px", padding: "16px 20px", background: "#f9fafb", borderRadius: "8px", border: "1px solid #e5e7eb" }}>
        ...
        {client?.contato && <p style={{ fontSize: "12px", color: "#6b7280", margin: "0 0 2px" }}>Contato: {client.contato}</p>}
        {client?.logradouro && (
        ...
        <tbody>
          {items.map((item, i) => (
            <tr key={item.id || i} style={{ background: i % 2 === 0 ? "#ffffff" : "#f9fafb" }}>
              ...
              <td style={{ padding: "10px 12px", fontSize: "12px", fontWeight: "600", border: "1px solid #e5e7eb", verticalAlign: "top" }}>{item.description}</td>
              ...
              <td style={{ padding: "10px 12px", textAlign: "center", fontSize: "12px", border: "1px solid #e5e7eb", verticalAlign: "top" }}>{item.quantity}</td>
              <td style={{ padding: "10px 12px", textAlign: "right", fontSize: "12px", border: "1px solid #e5e7eb", verticalAlign: "top" }}>{formatBRL(item.unit_price)}</td>
              <td style={{ padding: "10px 12px", textAlign: "right", fontSize: "12px", fontWeight: "600", border: "1px solid #e5e7eb", verticalAlign: "top" }}>{formatBRL(item.total_price)}</td>
            </tr>
          ))}
          {items.length === 0 && (
            <tr>
          ...
          {hasDiscount && (
            <>
              <tr>
                ...
                <td style={{ padding: "8px 12px", textAlign: "right", fontSize: "12px", border: "1px solid #e5e7eb" }}>{formatBRL(subtotal)}</td>
              </tr>
              <tr>
                <td colSpan={5} style={{ padding: "8px 12px", textAlign: "right", fontSize: "12px", color: "#dc2626", border: "1px solid #e5e7eb" }}>
                  Desconto {proposal?.discount_type === "percent" ? `(${proposal.discount_value}%)` : ""}
                </td>
                <td style={{ padding: "8px 12px", textAlign: "right", fontSize: "12px", color: "#dc2626", border: "1px solid #e5e7eb" }}>− {formatBRL(discountAmt)}</td>
              </tr>
            </>
          )}
